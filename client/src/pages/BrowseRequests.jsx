  ...
  return (
    <div className="page">
      <div className="browse-hero">
        ...
        <p>People are seeking referrals. Refer one at your company, upload proof, and earn RP.</p>
      ...
      <div className="mk-toggle">
        <Link to="/browse" className="active">🙋 Seeking referrals</Link>
        <Link to="/openings">🎁 Offering referrals</Link>
      </div>

      <div className="browse-toolbar">
        <span className="browse-count"><b>{loading ? '…' : view.length}</b> seeking</span>
        <div className="filter-chips">
          ...
          <button className={`fchip ${reward === 'low' ? 'active' : ''}`} onClick={() => setReward('low')}>Under 100</button>
        </div>
      ...
      {loading ? (
        ...
        <div className="empty">
          <div className="em">🕵️</div><h3>No one seeking right now</h3>
          <p>Check the Offering tab, or post your own.</p>
            ...
            return (
              <Link to={`/request/${r.id}`} key={r.id} className="job-card">
                <div className="job-head">
                  <div className="company-logo" style={{ background: pickColor(r.company_name) }}>{(r.company_name || '?')[0].toUpperCase()}</div>
                  <div className="job-title"><h3>{r.role_title}</h3><span className="co">🏢 {r.company_name}</span></div>
                </div>
                ...
                <div className="job-foot">
                  <div className="asker">
                    {asker.avatar_url ? <img className="asker-av" src={asker.avatar_url} alt="" /> : <div className="asker-av">{initials}</div>}
                    <div className="asker-info">
                      <b>{asker.full_name || 'Someone'}{asker.is_premium && <span title="Premium"> 💎</span>}</b>
                      <span><span className={`trust-dot ${trustLevel(asker.trust_score)}`} />Trust {asker.trust_score ?? 100}</span>
                    </div>
                  </div>
